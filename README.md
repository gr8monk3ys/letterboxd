...
Reviews can be generated by Claude, GPT or Gemini — whichever you have a key
for. Anthropic is the default and ships with the base install; the other two
are optional extras so you don't install three SDKs to use one.

```bash
uv sync --extra openai      # or: uv sync --extra gemini
uv run python -m src.reviewing.write_review --provider openai -n 5
```

Set a default with `AI_PROVIDER=openai` in `.env`.
